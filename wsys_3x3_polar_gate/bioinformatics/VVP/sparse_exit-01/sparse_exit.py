...
if len(sys.argv) != 3: 
    die("usage: sparse_exit.py hints.bin partial.json")

with open(sys.argv[1], 'rb') as f: 
    bloom = f.read(BLOOM_BYTES)

if len(bloom) != BLOOM_BYTES: 
    die("hint file must be 128 B")

try:
    with open(sys.argv[2]) as f: 
        edges = json.load(f)  # {"0":[1,2],"1":[0],...}
...
for u, vs in edges.items():
    for v in vs:
        if bloom_check(bloom, (u, str(v))):  # Convert v to string to match JSON
            G[u].append(str(v))
            G[str(v)].append(u)
